...
(defn insert-nth-A [arblist n x]
  (cond (or (< n 0) (> n (count arblist))) arblist
        (zero? n) (cons x arblist)
        :else (cons (first arblist) (insert-nth-A (rest arblist) (dec n) x))))

;; (insert-nth-A '(:a :b :c :d) 2 :x)
;; (insert-nth-A '(:a :b :c :d) 9 :x)
;; (insert-nth-A '(:a :b) 2 :x)
...
(defn inc-nth [arblist n]
  (if (and (>= n 0) (< n (count arblist))) ;;confirms that the index is in bounds
    (replace-nth arblist n (+ 1 (nth arblist n))) ;;uses replace-nth to replace the nth element with its incremented value
    arblist)) ;; returns the original list if index is out of bounds
